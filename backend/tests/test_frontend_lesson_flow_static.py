...
class FrontendLessonFlowStaticTests(unittest.TestCase):
    ...
    def test_correction_reveals_solution_after_explicit_action(self) -> None:
        ...
        self.assertIn("Voici ce que ton coup a permis.", correction)
    ...
    def test_summary_hides_audit_details_and_practice_is_state_driven(self) -> None:
        ...
        self.assertIn("Voir la leçon clé", self.summary)
        self.assertIn("Explorer", self.summary)
        self.assertIn("review-summary-dashboard", self.summary)
        self.assertIn("review-priority-action", self.summary)
