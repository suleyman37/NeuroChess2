...
$morningReportName = if ($MissionId -eq "A20AV") {
    ...
    "A20BB_TRUE_OVERNIGHT_SECOND_RUN_WITH_COMPOSER_FIRST_CLASSIFIER_REPORT.md"
} else {
...
$checks = [ordered]@{
    ...
    no_road_push = $true
    no_secrets = $true
    max_runtime_set = if ($MissionId -in @("A20AZ", "A20BB")) { ($MaxRuntimeMinutes -gt 0 -and $MaxRuntimeMinutes -le 520) } else { ($MaxRuntimeMinutes -gt 0 -and $MaxRuntimeMinutes -le 480) }
    max_iterations_set = if ($MissionId -in @("A20AZ", "A20BB")) { ($MaxIterations -gt 0 -and $MaxIterations -le 36) } else { ($MaxIterations -gt 0 -and $MaxIterations -le 20) }
}
...
$result = [ordered]@{
    ...
    recommended_next = if ($verdict -eq "NIGHT_NOT_READY") {
        if ($MissionId -eq "A20AV") { "A20AW_FINAL_NIGHT_READINESS_HARDENING" } elseif ($MissionId -eq "A20AW") { "A20AX_FINAL_AUTOPILOT_HARDENING_BEFORE_NIGHT" } elseif ($MissionId -eq "A20AY") { "A20AZ_FINAL_AUTOPILOT_REPORT_AND_HANDOFF" } elseif ($MissionId -eq "A20AZ") { "A20BA_LIVE_SUPERVISOR_REPAIR" } elseif ($MissionId -eq "A20BB") { "A20BC_GEMINI_3_5_FLASH_EXTENDED_WEB_LANE" } else { "A20AV_FIX_OMEGA_KERNEL" }
    } else {
        if ($MissionId -eq "A20AV") { "A20AW_FULL_NIGHT_PIXEL_REHEARSAL" } elseif ($MissionId -eq "A20AW") { "A20AX_FULL_NIGHT_REAL_RUN" } elseif ($MissionId -eq "A20AY") { "A20AZ_ROAD_TO_V2_MERGE_AUDIT_PLAN" } elseif ($MissionId -eq "A20AZ") { "A20BA_LIVE_SUPERVISOR_REPAIR" } elseif ($MissionId -eq "A20BB") { "A20BC_GEMINI_3_5_FLASH_EXTENDED_WEB_LANE" } else { "A20AV_LIMITED_AUTONOMOUS_PIXEL_REHEARSAL" }
    }
}
