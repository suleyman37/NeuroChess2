...
| `review_moment_importance_v1` | Categorical routing over existing Review signals into priority, secondary, micro-gap, good decision, informational, or no-major states. | `metrics/review_moment_importance.py`, `review_service._review_move_annotations`, `move_categories.build_review_sections`, `training_item_service._training_item_payload`, `review_practice_service._annotation_is_practice_eligible`. | Implemented as a non-numeric product-routing heuristic using move category, tags, win loss, move accuracy, internal criticality, missed gain, ply, book signal, player Win% band, and best move availability. | yes for documented v1 heuristic | medium | labels/reasons only | yes | Calibrate on a real annotated corpus; add explicit positive_gain persistence before treating good-decision coverage as complete. | Raw priority numbers are not exposed; micro-gaps/good decisions are not forced into Practice/training items by default. |
