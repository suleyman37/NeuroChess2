...
| `criticality_score_v1` | `win_loss * reliability * leverage * transition * persistence * novelty`; threshold 10, mate override, NMS. | `review_service.criticality_score`, `_select_review_moments`, `temporal_non_max_suppression`. | Same; `novelty_weight` currently 1.0; threshold 10; NMS window 2, override 1.5, max 5. | yes | info | debug/indirect | yes | Keep hidden as raw score; audit any UI that derives visible scores from it. | Drives Review moment and Practice candidate selection. |
...
| `accepted_moves_try_move_v0` | Not a public formula registry entry; docs require audit of accepted moves thresholds. | `metrics/try_move.py` | Best move accepted; other top moves accepted if delta from best player Win% <= 2 (`very_good`) or <= 5 (`acceptable`). | documented here only | medium | yes | yes in practice | Consider adding this to Metric Registry or Action/Formula docs if it remains training-driving. | Uses cached top moves only. |
...
Try Move accepted moves are determined from cached top moves:
...
be added to the formal formula/metric registry if it remains strategic.
