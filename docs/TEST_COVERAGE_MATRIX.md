...
| `backend/tests/test_review_explorer_cockpit_readability_static.py` | static | Explorer cockpit readability / context | Explorer line analysis labels, analysis presets, stable evaluation-bar placeholders, line-player dark style hooks, Today source fallback copy, and forbidden raw metric guard | Runtime board placement and actual line analysis calls | PASS targeted in P1 Explorer cockpit readability mission | medium/high | Keep paired with browser smokes because this is UX-sensitive |
...
| `scripts/browser_explorer_cockpit_actions_near_board_smoke.mjs` | browser smoke | Explorer cockpit action placement | After a local Explorer move, `Analyser ce coup`, `Analyser la ligne`, preset control, and the local-exploration notice remain visible near the board without horizontal overflow | Manual perception across more viewport sizes | PASS | high | Keep dock placement under manual review before pilot |
| `scripts/browser_explorer_analyze_line_smoke.mjs` | browser smoke | Explorer line analysis | Plays a short local branch, runs `Analyser la ligne`, receives a stable result, and verifies no Practice/training/Daily Plan side effects | Long branch UX beyond the V1 cap and every illegal branch variant | PASS | high | Extend only if branch analysis scope grows |
| `scripts/browser_eval_bar_stability_smoke.mjs` | browser smoke | Evaluation bar stability | Confirms the evaluation bar keeps stable dimensions and does not become visually disabled after an Explorer move | Human perception of very short transitions | PASS | high | Manual visual spot check still useful |
| `scripts/browser_today_start_context_smoke.mjs` | browser smoke | Today -> Commencer source context | Starts Practice from Today and confirms raw `coup indisponible` no longer appears when source move context is hydrated or falls back safely | Legacy real DB items with missing source move fields | PASS | high | Add legacy fixture if old production data is imported |
| `scripts/browser_line_player_readability_smoke.mjs` | browser smoke | Line player readability | Opens the line player and checks the dark readable theme, contrast, and absence of the prior light/blue treatment | Full visual taste review across devices | PASS | medium/high | Manual screenshot review remains useful |
