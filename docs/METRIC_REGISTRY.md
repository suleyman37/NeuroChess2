...
| review_moment_importance_v1 | Review moment importance | action | true | when_data_sufficient | heuristic_v0 | User-safe category for deciding whether a Review move is priority training, secondary training, micro-gap, good decision, informational, or no major moment. | win_loss_v1, move_accuracy_lichess_like_v1, criticality_score_v1, move category, tags, ply, book signal, best move availability | 1 analyzed move | Non-numeric product routing heuristic; labels may be shown but no raw priority number is exposed. Good decisions currently infer from quality/category, not explicit positive_gain. | implemented | review_moment_importance |
