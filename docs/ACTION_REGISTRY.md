...
| review.analyze_local_exploration_line | Analyser la ligne | app.review.board | Board exploration cockpit | secondary | local exploration branch has at least one move and no line analysis is running | exploration inactive, empty branch, or analysis already running | n/a | local exploration branch -> stable line feedback | fen_start, local moves_uci, analysis preset | calls `POST /api/review/explorer/evaluate-line`; may write bounded position-analysis cache only; never writes Practice attempts, training items, `due_at`, or Daily Plan | false | false | try_move_model_version | show `À recalculer` / `Non évalué` if stable analysis is unavailable; cap at 12 demi-coups in V1 | review_analyze_local_exploration_line | Must evaluate only the local branch, not the full game, and must not become Practice. |
...
- P1 Explorer cockpit readability follow-up: `review.analyze_local_exploration_move`
  and `review.analyze_local_exploration_line` are surfaced near the Review board
  once a local exploration move exists. Line analysis is capped to the local
  branch and shares the same side-effect boundary: no Practice attempts,
  training items, `due_at`, or Daily Plan rows. The analysis preset
  `fast` / `standard` / `deep` is an Explorer-only request hint; `standard`
  remains the default and must not expose engine internals in the normal UI.
